...
\code{z.score} provides the option to standardize factor exposures cross-sectionally across assets or across time periods. User can also choose to weight the factor exposures by some characteristic (ex: market cap) via \code{weight.var}.
...
fit.singleIndex <- fitFfm(asset.names=asset.names, factor.names="SP500.TR",
