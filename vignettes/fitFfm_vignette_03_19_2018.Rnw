...
The focus of this vignette is on the \code{fitFfm} function and related functionalities. The original function was designed by Doug Martin and initially implemented in S-PLUS by a number of University of Washington Ph.D. students: Christopher Green, Eric Aldrich, and Yindeng Jiang. Guy Yollin ported the function to R and Yi-An Chen modified that code. Sangeetha Srinivasan tested, and expanded the functionalities and S3 methods. Avinash Acharya and Chindhanai Uthaisaad further added new functionalities such as EWMA and GARCH errors, allowing for a market + industry model specification etc.
...
Here's an implementation of a stylized BARRA-type industry (sector) factor model for the 447 NYSE stocks described in section 1.3 earlier. Note that there are 10 mutually exclusive GICS sector classifications in any given time period.
<<>>=
# Sector Factor Model
...
<<>>=
fit.sector # print the fitted "tsfm" object
@

Looking at the estimated factor returns by sector, we note that the "Information Technology" sector had the highest average return (not suprising given the historical period covers most of the dot-com bubble).
