...
Figure 1 shows a scatter plot of residuals, with histograms, density overlays, correlations and significance stars. (Plot options are explained later in section 4.) Note the high residual correlation between MSFT and ORCL might be due to a sector/industry factor

<<fig.cap="Single factor model: Residual correlations">>=
